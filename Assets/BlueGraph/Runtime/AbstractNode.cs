...
namespace BlueGraph
{
    ...
    public abstract class AbstractNode : ISerializationCallbackReceiver
    {
        ...
        public object GetInputValue(string portName, object defaultValue = null)
        {
            ...
            if (!port.IsConnected)
            {
                Debug.Log("NOT CONNECTED! USING DEFAULT VALUE!");
                if (defaultValue is GameObject obj) {
                    Debug.Log(obj == null ? "NULL OBJECT" : obj.name);
                }
                if (defaultValue is MonoBehaviour beh) {
                    Debug.Log(beh == null ? "NULL BEHAVIOUR" : beh.name);
                }
                return defaultValue;
            }
